$FilePath = "C:\mywork\scm\CapitalPower.DocumentManagement\Dev\Main\CapitalPower.DM.Deployment\Application\Maintenance\dm-metadata-store_fromClient.bak"
...
Write-Message  "`r`n***  Term store import script for Capital Power Document Management Portal  ***`r`n" "cyan"
...
try{
	Import-ManagedMetadataStore $filePath $mmsAppName
	Write-Message "done." "cyan" $false
	exit 0
} catch {
    Write-Message "Failed to import term store. Error message: `"$_`"" "red"
	exit -1
}